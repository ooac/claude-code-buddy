...
- 哈希分支对齐：默认使用 `bun-exact(bun-wyhash)`，与 Claude 渲染逻辑一致
...
- 每次 `random/target` 仅同步写入 `companion` 的 soul 字段（`name/personality/hatchedAt`），骨架字段由种子实时推演。
- 每次切换会按当前物种重生成名字，避免“名称与当前宠物不对应”。
- 默认哈希模式是 `bun-exact(bun-wyhash)`；如需回退历史行为，可设置 `BUDDY_HASH_MODE=fnv` 后再运行命令。
- `card` 以“种子推演骨架 + companion soul”展示，不再依赖配置里的 bones 字段判断一致性。
- 运行态一致性诊断默认收敛到 `doctor` 命令，`random/target/card` 不再输出热更新强提示。
